@include('partials.header', ['title' => $course->course_code . ' - ' . 'Edit Module'])
...
<div class="main-container">
    ...
    <main class="main home w-full">
        @include('partials.navbar', ['page_title' => 'Courses > ' . $course->course_code . ' > ' . 'Edit Module' ])
        ...
        <section class="courses-main-section">
            ...
            <div class="section-header bg-white w-full h-[auto] rounded-md p-4 shadow-lg flex flex-col gap-2">
                <div class="bg-white breadcrumbs-student w-full h-aut py-2 px-4 rounded-md">
                    ...
                    <a href="" class="">> Edit {{ $module->name }}</a>
                </div>
            ...
            <div class="section-content flex flex-col gap-16 bg-white w-full h-[550px] rounded-md pt-8 pb-4 px-5 shadow-lg">
                ...
                <div class="flex flex-row flex-nowrap h-auto">
                    ...
                    <div class="form-container">
                        <form action="{{ route('teacher.modules.update',['course' => $course->id, 'module' => $module->id]) }}" method="POST" class="grid grid-cols-1 gap-8 w-full">
                            @csrf
                            @method('PUT')
                            <div class="flex flex-col">
                                <label for="" class="custom-label">Module Name</label>
                                <input type="text" name="name" id="" class="border-solid border-2 custom-input" value="{{ $module->name }}">
                            </div>
                            <div class="flex flex-col">
                                <label for="" class="custom-label">Description</label>
                                <textarea type="text" name="description" id="" rows="1" class="border-solid border-2 custom-input"> {{ $module->description }}</textarea>
                            </div>
                            <div class="flex flex-col">
                                <label for="" class="custom-label">Settings</label>
                                <div class="flex flex-nowrap flex-row gap-2">
                                    <input type="checkbox" name="is_task" value="checked" class="text-sm" {{ ($module->is_task) ? 'checked' : '' }}>Is Task</input>
                                </div>
                            </div>
