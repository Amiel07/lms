    ...
    <main class="main home w-full">
        ...
        <section class="courses-main-section">
                    ...
                    <div class="flex flex-row flex-nowrap gap-4">
                        ...
                        <div class="flex items-center">
                          <div class="flex flex-col h-auto gap-4">
                            <span class="font-bold text-4xl">{{ $assessment->assessment_name }}</span>
                            @php
                                $totalPoints = assessmentPoints($assessment);
                            @endphp
                            <p class="font-bold text-sm text-gray-400">TOTAL POINTS: {{ $totalPoints }}</p>
                            <p class="font-bold text-sm text-gray-400">{{ $assessment->direction }}</p>
            ...
            <div class="section-content flex flex-col gap-16 bg-white w-full h-[550px] rounded-md pt-8 pb-4 px-5 shadow-lg">
                ...
                <hr>
                <form action="{{route('assessment.download')}}" target="_blank" method="POST" >
                    @csrf
                    <input type="hidden" name="assessment_id" value="{{$assessment->id}}"></input>
                    <input type="hidden" name="course_id" value="{{$course->id}}"></input>
                    <button type="submit" class="uppercase hover:text-green-700">
                    ...
                    <table class="w-full border-collapse">
                        <thead>
                            <tr>
                                ...
                                <th class="px-2 py-1 text-start">Answers</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            @php
                                ...
                                $taken = isAssessmentDone($enrollee->student_id, $assessment->id);
                            @endphp
                            <tr class="hover:bg-blue-100">
                                <td class="px-2 py-1">{{ $fullname }}</td>
                                <td class="px-2 py-1 uppercase text-xs">
                                @if ($taken)
                                    <p class="text-green-800 font-bold">TEST TAKEN</p>
                                @else
                                    <p class="text-red-400 font-bold">TEST NOT TAKEN</p>
                                @endif    
                                </td>
                                {{-- <td class="px-2 py-1 uppercase text-xs text-start">
                                    
                                </td> --}}
                                <td class="px-2 py-1 uppercase text-xs text-start flex gap-8 flex-wrap">
                                        <form action="{{route('assessment.viewanswer')}}" target="_blank" method="POST" >
                                            @csrf
                                            <input type="hidden" name="student_id" value="{{$enrollee->student_id}}"></input>
                                            <input type="hidden" name="assessment_id" value="{{$assessment->id}}"></input>
                                            <button type="submit" class="uppercase hover:text-green-700">
